...
\usepackage{float} % posicionamento da figura
\usepackage{multirow} % mesclar linhas da tabela
\usepackage{tabularx} % margem da tabela
\usepackage{multicol}

\setlength{\columnsep}{5mm} % espaço entre duas colunas
\setlength{\columnseprule}{0.5mm} % adiciona o separador entre as colunas
\begin{document}
	\title{\textbf{{\Huge Capa e sumário}}}
	\author{Alexandre Rogério}
	%\date{Outubro de 2021}
	\maketitle % Cria o título, o autor e a data
	\thispagestyle{empty} % oculta a numeração da página
	\newpage
	
	\setcounter{page}{1} % começa a contar as páginas
	\pagenumbering{Roman} % algarismo romano ou arábico
	\tableofcontents % Criar sumário ( cao o sumário não atualize, o indicado é compilar o arquivo duas vezes )
	\newpage
